...
// EXTERNAL MODULE: ../../node_modules/.pnpm/@actions+artifact@6.2.1_supports-color@7.2.0/node_modules/@actions/artifact/lib/artifact.js + 326 modules
var lib_artifact = __webpack_require__(95960);
// EXTERNAL MODULE: ../../node_modules/.pnpm/@actions+core@3.0.1/node_modules/@actions/core/lib/core.js + 11 modules
var core = __webpack_require__(34785);
...
var dist = __webpack_require__(87922);
...
const constants_SCHEMA_VERSION = "1.0.0";
const constants_NON_CONFORMANCE_DISCLAIMER = "Automated and agent-assisted testing identifies a subset of accessibility barriers. This result is not a declaration of WCAG conformance.";
...
const constants_DEFAULT_VIEWPORT = { width: 1280, height: 720 };
const constants_DEFAULT_TIMEOUT_MS = 30_000;
const MAX_HTML_EVIDENCE_LENGTH = 800;
const constants_MAX_REDIRECTS = 10;
const constants_MAX_HTML_INPUT_BYTES = 1_000_000;
...
const waiters = [];
async function concurrency_withBrowserSlot(operation) {
...
var external_axe_core_ = __webpack_require__(67411);
...
function fingerprint_normalizeRoute(value) {
    try {
...
function createFindingFingerprint(input) {
    const material = [
        fingerprint_normalizeRoute(input.route),
...
// EXTERNAL MODULE: ../../node_modules/.pnpm/zod@4.5.4/node_modules/zod/v4/classic/schemas.js + 17 modules
var schemas = __webpack_require__(34041);
...
const auditReportSchema = schemas/* object */.Ik({
    schemaVersion: schemas/* literal */.eu(constants_SCHEMA_VERSION),
    ...
    disclaimer: schemas/* literal */.eu(constants_NON_CONFORMANCE_DISCLAIMER)
})
    .strict();
...
const comparisonReportSchema = schemas/* object */.Ik({
    ...
    .strict();
const schema_verificationReportSchema = schemas/* object */.Ik({
    schemaVersion: schemas/* literal */.eu(constants_SCHEMA_VERSION),
...
const schema_keyboardJourneySchema = schemas/* object */.Ik({
...
const journeyStepResultSchema = schemas/* object */.Ik({
    ...
    .strict();
const schema_journeyReportSchema = schemas/* object */.Ik({
    schemaVersion: schemas/* literal */.eu(constants_SCHEMA_VERSION),
...
var external_node_dns_promises_ = __webpack_require__(71553);
// EXTERNAL MODULE: ../../node_modules/.pnpm/ipaddr.js@2.5.0/node_modules/ipaddr.js/lib/ipaddr.js
var ipaddr = __webpack_require__(31633);
...
async function security_assertSafeUrl(rawUrl, allowPrivateNetwork = false) {
    let url;
    try {
        url = new URL(rawUrl);
    }
    ...
    try {
        addresses = ipaddr.isValid(hostname)
            ? [hostname]
            : (await (0,external_node_dns_promises_.lookup)(hostname, { all: true, verbatim: true })).map((result) => result.address);
    }
    catch {
...
async function security_authenticationSensitiveValues(headers, storageState) {
    const headerValues = Object.values(headers ?? {});
    if (typeof storageState === "string") {
        try {
            const parsed = JSON.parse(await (0,promises_.readFile)(storageState, "utf8"));
...
function security_authenticationMode(headers, storageState) {
...
function normalize_normalizeAxeResults(options) {
    const route = options.routeLabel ?? fingerprint_normalizeRoute(options.finalUrl);
    ...
    return auditReportSchema.parse({
        ...
        disclaimer: constants_NON_CONFORMANCE_DISCLAIMER
    });
}
...
async function browser_scanPage(options) {
    return concurrency_withBrowserSlot(async () => {
        const name = options.browserName ?? "chromium";
        const viewport = options.viewport ?? { ...constants_DEFAULT_VIEWPORT };
        const timeoutMs = options.timeoutMs ?? constants_DEFAULT_TIMEOUT_MS;
        const maxRedirects = options.maxRedirects ?? constants_MAX_REDIRECTS;
        const sensitiveValues = await security_authenticationSensitiveValues(options.headers, options.storageState);
        await security_assertSafeUrl(options.url, options.allowPrivateNetwork ?? false);
        ...
        try {
            ...
            await context.route("**/*", async (route) => {
                ...
                try {
                    ...
                    await security_assertSafeUrl(requestUrl, options.allowPrivateNetwork ?? false);
                    await route.continue();
                }
                catch (error) {
            ...
            if (redirectCount(response) > maxRedirects)
                throw new Error(`Redirect limit of ${maxRedirects} exceeded.`);
            await security_assertSafeUrl(page.url(), options.allowPrivateNetwork ?? false);
            ...
            return normalize_normalizeAxeResults({
                results,
                ...
                include: options.include,
                exclude: options.exclude,
                authenticationMode: security_authenticationMode(options.headers, options.storageState),
                sensitiveValues
            });
        }
        finally {
...
function scopeSignature(report) {
    return JSON.stringify({
        route: fingerprint_normalizeRoute(report.route),
...
function compare_compareReports(beforeInput, afterInput) {
            ...
            for (const [index, action] of journey.actions.entries()) {
                ...
                try {
                    ...
                    steps.push({
                        ...
                        focusedElement: evidence.selector,
                        accessibleName: redactSecrets(evidence.name, sensitiveValues),
                        failureReason: redactSecrets(error instanceof Error ? error.message : "Journey action failed.", sensitiveValues),
                        durationMs: Math.max(0, performance.now() - started)
                    });
                }
            }
...
function deepRedact(value) {
    if (typeof value === "string")
        return security_redactSecrets(value);
...
;// CONCATENATED MODULE: ./dist-types/index.js
...
async function run() {
    const url = core/* getInput */.V4("url", { required: true });
    const stateLabel = core/* getInput */.V4("state-label") || "default";
    const routeLabel = core/* getInput */.V4("route-label");
    const readySelector = core/* getInput */.V4("ready-selector");
    const tags = (core/* getInput */.V4("axe-tags") || "wcag2a,wcag2aa,wcag21a,wcag21aa,wcag22aa")
        .split(",")
        .map((tag) => tag.trim())
        .filter(Boolean);
    const threshold = core/* getInput */.V4("fail-on");
    if (!thresholds.has(threshold))
        throw new Error(`Unsupported fail-on threshold: ${threshold}`);
    const outputDirectory = (0,external_node_path_.resolve)(core/* getInput */.V4("output-directory") || ".a11y-agent/action");
    await (0,promises_.mkdir)(outputDirectory, { recursive: true });
    const after = await browser_scanPage({
        url,
        stateLabel,
        wcagTags: tags,
        allowPrivateNetwork: core/* getBooleanInput */.Vt("allow-private-network"),
        ...(routeLabel ? { routeLabel } : {}),
        ...(readySelector ? { readySelector } : {})
    });
    const baselinePath = core/* getInput */.V4("baseline");
    let report = after;
    let candidates = after.findings;
    if (baselinePath) {
        const baseline = auditReportSchema.parse(JSON.parse(await (0,promises_.readFile)((0,external_node_path_.resolve)(baselinePath), "utf8")));
        const comparison = compare_compareReports(baseline, after);
        report = comparison;
        candidates = comparison.new;
    }
    const jsonPath = (0,external_node_path_.resolve)(outputDirectory, "a11y-agent-report.json");
    const sarifPath = (0,external_node_path_.resolve)(outputDirectory, "a11y-agent-report.sarif");
    await Promise.all([
        (0,promises_.writeFile)(jsonPath, formatJson(report), { encoding: "utf8", mode: 0o600 }),
        (0,promises_.writeFile)(sarifPath, formatSarif(report), { encoding: "utf8", mode: 0o600 })
    ]);
    const artifact = new lib_artifact/* DefaultArtifactClient */.t9();
    await artifact.uploadArtifact("a11y-agent-reports", [jsonPath, sarifPath], outputDirectory);
    const counts = report.reportType === "comparison"
        ? report.counts
        : {
    ...
    await core/* summary */.z
        ...
        .addQuote("Automated and agent-assisted testing identifies a subset of accessibility barriers. This result is not a declaration of WCAG conformance.")
        .write();
    const failures = findingsAtOrAbove(candidates, threshold);
    if (failures.length > 0)
        core/* setFailed */.C1(`${failures.length} new finding(s) met the explicit ${threshold} threshold.`);
}
run().catch((error) => core/* setFailed */.C1(error instanceof Error ? error.message : "a11y-agent Action failed."));
//# sourceMappingURL=index.js.map
