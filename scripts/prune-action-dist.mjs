...
for (const entry of await readdir(actionDist, { withFileTypes: true })) {
  if (!entry.name.endsWith(".d.ts") && !entry.name.endsWith(".d.ts.map"))
    continue;
  await rm(resolve(actionDist, entry.name), { force: true });
}
...
const axeTarget = resolve(actionDist, "node_modules/axe-core");
